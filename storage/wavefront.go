...
func newStorage(proxyAddress string, prefix string, interval int, addTags string, sourceTag string, taggifyLabels bool, wfDebug bool) (*wavefrontStorage, error) {
...
func (driver *wavefrontStorage) AddStats(ref info.ContainerReference, stats *info.ContainerStats) error {
	...
	if strings.Contains(source,"{") && strings.Contains(source, "}") {
		...
		labelKey = strings.TrimRight(source,"}")
		labelVal := ref.Labels[labelKey]
		hostname, _ := os.Hostname()
		newSource := labelVal + hostname
		source = newSource
	}
